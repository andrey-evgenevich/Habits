...
class NotificationModelTest(TestCase):
    def setUp(self):
        ...
        self.habit = Habit.objects.create(
            user=self.user, place="Дом", time="08:00:00", action="Зарядка", duration=30
        )

    def test_notification_creation(self):
        notification = Notification.objects.create(
            user=self.user, habit=self.habit, scheduled_time=timezone.now()
        )
    ...
    def test_status_choices(self):
        ...
        self.assertIn(
            notification.status, [choice[0] for choice in Notification.Status.choices]
        )
    ...
    def test_default_ordering(self):
        ...
        self.assertGreater(
            notifications[0].scheduled_time, notifications[1].scheduled_time
        )
...
class NotificationSerializerTest(APITestCase):
    def setUp(self):
        ...
        self.habit = Habit.objects.create(
            user=self.user, place="Дом", time=time(8, 0), action="Зарядка", duration=30
        )
...
class NotificationViewTest(TestCase):
    ...
    def test_user_sees_only_own_notifications(self):
        Notification.objects.create(
            user=self.other_user, habit=self.habit, scheduled_time=timezone.now()
        )
        response = self.client.get("/api/notifications/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["results"]), 1)
